...
function controlsHint() {
  return input.isTouch() || matchMedia('(pointer: coarse)').matches
    ...
    : 'WASD — ИДТИ. ← РУКА, → НОГА, ↑ БРОСОК. РУКА БЬЁТ НОГУ, НОГА — БРОСОК, БРОСОК — РУКУ; ОДИНАКОВЫЕ ГАСЯТ ДРУГ ДРУГА. НАД ВРАГОМ ГОРИТ ЕГО ПРИЁМ — ОТВЕЧАЙ ТЕМ, ЧТО ЕГО БЬЁТ. ПРОБЕЛ — ВЗЯТЬ ИЛИ БРОСИТЬ ОРУЖИЕ. R — ЗАНОВО.';
}
