...
function controlsHint() {
  return input.isTouch() || matchMedia('(pointer: coarse)').matches
    ...
    : 'WASD — ИДТИ. ← РУКА, → НОГА, ↑ БРОСОК. БЕЗОРУЖНОГО КЛАДУТ ДВА УДАРА РУКОЙ ИЛИ ОДИН НОГОЙ, НО НОГА МЕДЛЕННЕЕ. ОРУЖИЕ УБИВАЕТ С ОДНОГО КАСАНИЯ — И ТЕБЯ ТОЖЕ. ПРОБЕЛ — ВЗЯТЬ ИЛИ БРОСИТЬ. R — ЗАНОВО.';
}
