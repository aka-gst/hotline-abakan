...
export function createAssets(base = 'assets/') {
   ...
   *
   * Поставки приходили с правильными размерами и пустым нутром: тридцать
   * одинаковых кадров, залитых одной краской. Движок клал такое поверх
   * процедурной графики и делал игру хуже, молча. Тот же счёт, что в
   * tests/assets.mjs, только короче: заливка вместо рисунка или ходьба,
   * которая не двигается, — лист не берём.
   *
   * По числу цветов человека не распознать: размытая фигура с неоновым
   * свечением набирает их сотнями, а честный плоский пиксель-арт
   * обходится четырьмя. Поэтому здесь остался только пол — меньше трёх
   * цветов не рисунок, а заливка.
   *
