...
const OVERLAY = new Set(['wall_edge', 'wall_corner', 'door_h', 'door_v', 'pipe', 'rug', 'vent',
  /* Стол и выход тоже ложатся на пол, а не вместо него: стол занимает
     середину клетки, выход — знак поверх плитки. Требовать от них
     закрашенной клетки целиком значит требовать закрасить пол под ними. */
  'table', 'exit', 'glass']);
